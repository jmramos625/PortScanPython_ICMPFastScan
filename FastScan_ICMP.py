import random
import socket  # criação de conexão e envio de pacotes
import time  # tempo de conexões e função relacionadas a tempo
import ipaddress  # criação de alteração de configurações de IP
import struct  # tradução de objetos do pyhton para binário
from threading import Thread  # biblioteca para aumentar a velocidade das conexões e requisições (simultâneo)

SIGNAL = True  # para determinar quando deve continar executando o serviço ou não
# como é separado em duas threads, ele determina quando para uma ou outra
...
def ping(addr, timeout=1):
    try:
        # socket IPv4, do tipo RAW(cru), porém usando o protocolo ICMP
